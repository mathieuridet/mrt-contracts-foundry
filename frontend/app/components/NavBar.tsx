  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 lg:px-6">
        <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2 text-base font-semibold tracking-tight lg:text-lg">
        ...
        <nav className="hidden shrink-0 items-center gap-1 rounded-full border border-slate-200 bg-white/80 px-2 py-1 text-sm shadow-sm lg:flex">
        ...
        <div className="flex shrink-0 items-center gap-3">
          <div className="relative hidden items-center gap-2 lg:flex">
            <button
              ...
              className="inline-flex min-w-[140px] items-center justify-between gap-2 rounded-full border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm transition-colors hover:bg-slate-100"
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span className="whitespace-nowrap">{activeChain.label}</span>
              </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="currentColor"
                className={`h-3 w-3 shrink-0 transition-transform ${chainMenuOpen ? "rotate-180" : ""}`}
              >
                <path
                  fillRule="evenodd"
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white p-2 text-slate-600 transition-colors hover:bg-slate-100 lg:hidden"
              onClick={() => setOpen((v) => !v)}
